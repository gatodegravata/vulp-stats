...
-------------------------------------------------------------------------
-- MERCADOS EUROPEUS (Over/Under Padrão - CORRIGIDO)
-------------------------------------------------------------------------
    
-- Linha 0.5 Gols (Mercado 106)
MAX(price) FILTER (WHERE market = '106' AND outcome = '106' AND ord = 1) AS "over05_abertura",
MAX(price) FILTER (WHERE market = '106' AND outcome = '106' AND is_last) AS "over05_fechamento",
MAX(price) FILTER (WHERE market = '106' AND outcome = '107' AND ord = 1) AS "under05_abertura",
MAX(price) FILTER (WHERE market = '106' AND outcome = '107' AND is_last) AS "under05_fechamento",

-- Linha 1.5 Gols (Mercado 108)
MAX(price) FILTER (WHERE market = '108' AND outcome = '108' AND ord = 1) AS "over15_abertura",
MAX(price) FILTER (WHERE market = '108' AND outcome = '108' AND is_last) AS "over15_fechamento",
MAX(price) FILTER (WHERE market = '108' AND outcome = '109' AND ord = 1) AS "under15_abertura",
MAX(price) FILTER (WHERE market = '108' AND outcome = '109' AND is_last) AS "under15_fechamento",
...
MAX(price) FILTER (WHERE market = '1014' AND outcome = '1015' AND is_last) AS "under45_fechamento",
    ...
    -- Correct Score (Mercado 10336) - Retorna um mapa JSON contendo todos os placares disponíveis e suas respectivas odds
    jsonb_object_agg(outcome, price) FILTER (WHERE market = '10336' AND is_last) AS "correct_score_fechamento"
...
WHERE j.start_time >= '2026-01-01 00:00:00+00'
  AND j.start_time <= '2027-05-26 23:59:59+00'
  AND j.participant1_name NOT LIKE '%SRL%'
  AND j.category_slug <> 'soccerspecials'
