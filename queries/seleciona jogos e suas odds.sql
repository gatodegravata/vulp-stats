...
JOIN jogos_odds o 
 ...
 AND COALESCE(o.odds_brutas->'bookmakers'->'bet365'->'markets', '{}'::jsonb) ? '104'

LEFT JOIN LATERAL (
    SELECT 
        m.key AS market,
        oc.key AS outcome,
        (pl.pl_elem->>'price')::numeric AS price,
        pl.ord,
        pl.is_last
    ...
    JOIN LATERAL (
        SELECT 
            pl_elem,
            ROW_NUMBER() OVER (
                PARTITION BY m.key, oc.key
                ORDER BY (pl_elem->>'createdAt')::timestamp
            ) AS ord,
            ROW_NUMBER() OVER (
                PARTITION BY m.key, oc.key
...
WHERE j.start_time >= '2025-10-01 00:00:00+00'
  AND j.start_time <= '2026-05-05 23:59:59+00'
